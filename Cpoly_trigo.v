...
Require Import under Reals Coquelicot.Coquelicot Psatz CPoly Rstruct Poly_complements seq_base.
Import GRing.Theory.
...
Definition pT_fun n x := cos (INR n * acos x).

Lemma pT_fun_0 x : -1 <= x <= 1 -> pT_fun 0 x = 1.
Proof. by rewrite /pT_fun Rmult_0_l cos_0. Qed.

Lemma pT_fun_1 x : -1 <= x <= 1 -> pT_fun 1 x = x.
Proof. by rewrite /pT_fun Rmult_1_l; exact: acos_right_inv. Qed.

Lemma pT_fun_rec x n : 
  -1 <= x <= 1 ->  pT_fun n.+2 x = 2 * x * pT_fun n.+1 x - pT_fun n x.
Proof.
move=> H.
suff : pT_fun n.+2 x + pT_fun n x = 2 * x * pT_fun n.+1 x.
  move=> <-; ring.
rewrite form1.
...
Lemma pT_fun_pT n (x: R):
	-1<= x <= 1 -> pT_fun n x = ('T_n).[x].
Proof.
move => ineq.
elim: n {-2}(n) (leqnn n) => [n ass | n ih k ineqk].
	have/eqP-> : n == 0%nat by rewrite -leqn0.
	by rewrite pT_fun_0; try lra; rewrite pT0 hornerC.
rewrite leq_eqVlt in ineqk; case/orP: ineqk => [/eqP eqn | ineqk]; last by rewrite ih.
case E: n => [ | m]; first by	rewrite eqn E pT_fun_1; try lra; first by rewrite pT1 hornerX.
rewrite eqn E pTSS pT_fun_rec; try lra.
rewrite hornerD hornerM mulr2n hornerD hornerX hornerN.
by rewrite -!ih; toR; try lra; rewrite E.
Qed.

Lemma pT_fun_cos n a : 0 <= a <= PI ->  ('T_n).[cos a] = cos ((INR n) * a).
Proof.
by move=> ineq; rewrite -pT_fun_pT /pT_fun; [rewrite acos_left_inv | apply: COS_bound].
Qed.
...
Lemma pT_fun_comp m n x : -1 <= x <= 1 -> (pT_fun n (pT_fun m x)) = pT_fun (n * m) x.
Proof.
move=> Ha.
have U := acos_bound _ Ha.
rewrite /pT_fun mult_INR Rmult_assoc.
set v := _ * acos x.
have HP := PI2_1;
...
(* This Lemma is here and not in CPoly since comp_polyM requires the ring
to be commutative (I think it should need that assumption). *)
Lemma pT_comp n m: 'T_n \Po 'T_m = 'T_(n * m) :> {poly R}.
Proof.
move: n; apply induc2 => [ | | n ih1 ih2].
		by rewrite pT0 comp_polyC.
	by rewrite pT1 comp_polyX mul1n.
rewrite pTSS comp_polyB -mul2.
rewrite comp_polyM comp_polyZ comp_polyX.
rewrite ih1 ih2 mul_pT.
rewrite -mulSn -[RHS]addr0 -addrA; f_equal.
by rewrite mulSn absnC absnnD subrr.
Qed.
