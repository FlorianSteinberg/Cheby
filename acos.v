...
Lemma pT_fun_pT n (x: R):
	(Rle (-R1) x) /\ (Rle x R1) -> pT_fun n x = ('T_n).[x].
Proof.
move => ineq.
...
Lemma pT_fun_cos n a : 0 <= a <= PI ->  pT_fun n (cos a) = cos ((INR n) * a).
Proof. move=> *; rewrite /pT_fun acos_left_inv //. Qed.
End pT_trigo.
...
Lemma pT_compi m n a : -1 <= a <= 1 -> pT_fun n (pT_fun m a) = pT_fun (n * m) a.
Proof.
move=> Ha.
